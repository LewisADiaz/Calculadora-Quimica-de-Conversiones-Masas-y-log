...
string obtenerHoraActual() {
    time_t ahora = time(0);
    struct tm* tiempoSeguro = localtime(&ahora);
    char buffer[10];
...
int main() {
    ...
    do {
        ...
        case 3: {
            ...
            if (opcionCategoria == 1) { mostrarElementos(metalesAlcalinos, CANTIDAD_ALCALINOS); 
            cin >> elemento1; masaElemento1 = masasAlcalinos[elemento1 - 1]; }
            else if (opcionCategoria == 2) { mostrarElementos(alcalinoterreos, CANTIDAD_ALCALINOTERREOS); 
            cin >> elemento1; masaElemento1 = masasAlcalinoterreos[elemento1 - 1]; }
            else if (opcionCategoria == 3) { mostrarElementos(metalesTransicion, CANTIDAD_TRANSICION); 
            cin >> elemento1; masaElemento1 = masasTransicion[elemento1 - 1]; }
            else if (opcionCategoria == 4) { mostrarElementos(metaloides, CANTIDAD_METALOIDES); 
            cin >> elemento1; masaElemento1 = masasMetaloides[elemento1 - 1]; }
            else if (opcionCategoria == 5) { mostrarElementos(noMetales, CANTIDAD_NOMETAL); 
            cin >> elemento1; masaElemento1 = masasNoMetales[elemento1 - 1]; }
            else if (opcionCategoria == 6) { mostrarElementos(halogenos, CANTIDAD_HALOGENOS); 
            cin >> elemento1; masaElemento1 = masasHalogenos[elemento1 - 1]; }
            else if (opcionCategoria == 7) { mostrarElementos(gasesNobles, CANTIDAD_GASES); 
